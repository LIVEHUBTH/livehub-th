    ...
    for (
      ...
      packageItem.session_ids =
        (linkedResult.results || [])
          .map(
            item =>
              cleanText(item.session_id)
          )
          .filter(Boolean);

      packageItem.price =
        Number(packageItem.price || 0);
...
async function startViewingSession(
  ...
  /*
   * ปิด session เก่าของเครื่องเดิม
   */
 ...
 * =========================================
 * PACKAGE INPUT
 * สำคัญ: มีฟังก์ชันนี้เพียงชุดเดียว
 * =========================================
 */
